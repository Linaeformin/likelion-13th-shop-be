...
public class ItemController {
    ...
    public ResponseEntity<Long> createItem(@RequestPart(name = "itemFormDto") ItemFormDto itemFormDto, @RequestPart(value = "itemImg", required = false) MultipartFile itemImg) {
        System.out.println("itemFormDto"+itemFormDto);
        if (itemImg == null) {
            System.out.println("itemImg is null");
            try {
                // 새로운 아이템 저장
                Long itemId = itemService.saveItem(itemFormDto); //이미지 없는 상품 등록
                // 저장된 아이템의 아이디 반환
    ...
    // 기존 상품 수정
    @PatchMapping("/{itemId}")
    public ResponseEntity<String> updateItem(@PathVariable("itemId") Long itemId, @RequestPart(name = "itemFormDto") ItemFormDto itemFormDto, @RequestPart(value = "itemImg", required = false) MultipartFile itemImg){
        try {
            // 상품 수정
