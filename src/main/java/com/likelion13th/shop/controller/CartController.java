...
public class CartController {
    ...
    @PostMapping("/add")
    public ResponseEntity<String> addCartItems(@RequestParam("email") String email, @RequestBody List<CartItemDto> cartItemDtos){
    ...
    public ResponseEntity<String> updateCartItemCount(@RequestParam("email") String email, @RequestBody CartDetailDto cartDetailDto){
        try {
            cartService.updateCartItem(email, cartDetailDto);
            return ResponseEntity.ok("상품 ID: " + cartDetailDto.getItemId() + ", 수량: " + cartDetailDto.getItemCount());
        } catch (EntityNotFoundException e ){
    ...
    public ResponseEntity<String> orderCartItem(@RequestParam("email") String email, @RequestBody CartOrderDto cartOrderDto) {
        List<CartOrderDto> cartOrderDtoList = cartOrderDto.getCartOrderDtos();  // ✅ 여기서 리스트 꺼냄
